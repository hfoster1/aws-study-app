...
const WEEKS = [
  ...
  {
    week: 2,
    title: "IAM Deep Dive & Encryption Architecture",
    domain: "Design Secure Architectures",
    domainWeight: "30%",
    accent: "#EF4444",
    status: "summary",
...
const DomainBar = () => {
  const domains = [
    ...
    { name: "High-Performing", weight: 24, color: "#10B981" },
    { name: "Cost-Optimized", weight: 20, color: "#F59E0B" }
  ];
  return (
    <div style={{ marginBottom: "2.5rem" }}>
      <div style={{ display: "flex", borderRadius: 8, overflow: "hidden", height: 10, marginBottom: "0.75rem" }}>
        {domains.map((d) => (
          <div key={d.name} style={{ width: `${d.weight}%`, background: d.color, transition: "width 0.6s ease" }} />
        ))}
      </div>
      <div style={{ display: "flex", gap: "1.25rem", flexWrap: "wrap" }}>
        {domains.map((d) => (
          <div key={d.name} style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
            ...
            <span style={{ color: "#94A3B8", fontSize: "0.7rem", fontFamily: "'DM Mono', monospace" }}>
              {d.name} ({d.weight}%)
            </span>
          </div>
        ))}
  ...
  return (
    <div
      style={{
        ...
        borderColor: open ? "#F9731666" : "#334155"
      }}
    >
      <button
      ...
      >
        ...
        <span style={{ color: "#E2E8F0", fontSize: "0.9rem", fontWeight: 500, flex: 1 }}>
          {concept.title}
        </span>
        <span
          style={{
      ...
      {open && (
        <div style={{ padding: "0 1.25rem 1.25rem 3.25rem" }}>
          <p style={{ color: "#CBD5E1", fontSize: "0.85rem", lineHeight: 1.7, margin: "0 0 1rem 0" }}>
            {concept.detail}
          </p>
          <div
            style={{
          ...
          >
            ...
            <p style={{ color: "#94A3B8", fontSize: "0.8rem", lineHeight: 1.6, margin: "0.3rem 0 0 0" }}>
              {concept.exam}
            </p>
          </div>
        </div>
  ...
  return (
    <div
      style={{
        ...
        overflow: "hidden",
        transition: "border-color 0.2s",
        borderColor: open ? "#10B98144" : "#334155"
      }}
    >
      <button
        onClick={() => setOpen(!open)}
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ color: "#E2E8F0", fontSize: "0.9rem", fontWeight: 500 }}>{lab.title}</div>
          <div style={{ display: "flex", gap: "0.6rem", marginTop: "0.3rem" }}>
            <span
              style={{
                color: diffColors[lab.difficulty] || "#94A3B8",
                fontSize: "0.65rem",
                fontFamily: "'DM Mono', monospace"
              }}
            >
            ...
            <span style={{ color: "#475569", fontSize: "0.65rem", fontFamily: "'DM Mono', monospace" }}>
              ~{lab.time}
            </span>
          </div>
        </div>
      ...
      {open && (
        <div style={{ padding: "0 1.25rem 1.25rem 3rem" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", marginBottom: "1rem" }}>
            {lab.steps.map((step, i) => (
              <div key={i} style={{ display: "flex", gap: "0.6rem", alignItems: "flex-start" }}>
                ...
                <span style={{ color: "#CBD5E1", fontSize: "0.82rem", lineHeight: 1.6 }}>{step}</span>
              </div>
            ))}
          </div>
          <div
          ...
          >
            <span style={{ color: "#10B981", fontSize: "0.8rem", flexShrink: 0 }}>↳</span>
...
const WeekDetailed = ({ week }) => (
  <div>
    ...
    <div style={{ marginBottom: "2.5rem" }}>
      ...
      >
        Hands-On Labs (AWS Console)
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
        {week.labs.map((l, i) => (
          <LabCard key={i} lab={l} index={i} />
        ))}
      </div>
    </div>
        ...
        {week.checkpoint.map((c, i) => (
          <div
            ...
            style={{
              ...
              gap: "0.6rem",
              alignItems: "flex-start",
              marginBottom: i < week.checkpoint.length - 1 ? "0.75rem" : 0
            }}
          >
            <span style={{ color: "#F97316", flexShrink: 0, fontSize: "0.9rem" }}>◇</span>
            <span style={{ color: "#CBD5E1", fontSize: "0.85rem", lineHeight: 1.6 }}>{c}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
        {week.topics.map((t, i) => (
          <div key={i} style={{ display: "flex", gap: "0.6rem", alignItems: "flex-start" }}>
            ...
            <span style={{ color: "#CBD5E1", fontSize: "0.82rem", lineHeight: 1.6 }}>{t}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    >
      ...
      >
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          ...
          >
            Exam prep combined with hands-on console labs. Each week builds real architecture skills, not just exam knowledge.
          </p>
          <DomainBar />
        </div>
      </div>
      ...
      {/* Week nav + content */}
      <div style={{ maxWidth: 960, margin: "0 auto", display: "flex", minHeight: "calc(100vh - 240px)" }}>
        {/* Sidebar */}
        <div
          style={{
            width: 220,
        ...
        >
            ...
            >
              <span
                style={{
                  fontFamily: "'DM Mono', monospace",
                  fontSize: "0.6rem",
                  color: i === activeWeek ? wk.accent : "#475569",
                  flexShrink: 0,
                  marginTop: "0.15rem",
                  width: 14
                }}
              ...
              <span
                style={{
                  color: i === activeWeek ? "#E2E8F0" : "#94A3B8",
                  fontSize: "0.78rem",
                  lineHeight: 1.4,
                  fontWeight: i === activeWeek ? 500 : 400
                }}
              >
                {wk.title}
              </span>
          ...
          >
            ...
            <div style={{ color: "#CBD5E1", fontSize: "0.72rem", lineHeight: 1.5 }}>
              CCP passed ✓
              <br />
              50% exam voucher ready
              <br />
              Custom exam tool built
            </div>
          </div>
        </div>

        {/* Content */}
        <div ref={contentRef} style={{ flex: 1, padding: "2rem 2.5rem", overflowY: "auto" }}>
          {/* Week header */}
          ...
          >
            ...
            {activeWeek < WEEKS.length - 1 && (
              <button
                onClick={() => setActiveWeek(activeWeek + 1)}
                style={{
                  background: WEEKS[activeWeek + 1].accent + "22",
                  border: `1px solid ${WEEKS[activeWeek + 1].accent}44`,
                  borderRadius: 8,
                  padding: "0.6rem 1.25rem",
                  color: WEEKS[activeWeek + 1].accent,
                  cursor: "pointer",
                  fontFamily: "'DM Mono', monospace",
                  fontSize: "0.78rem"
                }}
              >
                Week {activeWeek + 2}: {WEEKS[activeWeek + 1].title} →
              </button>
            )}
          </div>
        </div>
